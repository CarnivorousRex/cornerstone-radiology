...
//  $('#recentPages').append($("<option></option>").attr("value","test").text($('#pageTo').val()));
    ...
    //$("#recentPages").empty().append(option); 
    
    
    
//    $('#recentPages').options[this.length]= new Option($('#pageTo').val();
return true;
}    
    
    //$('#sendButton').click(function () {
      //  window.open("www.gmail.com");
        //document.sendmessage.recentPages[document.sendmessage.recentPages.length]= new Option($('#pageTo').val());
    //    $('#recentPages').options[this.length]= new Option($('#pageTo').val();
    //})
        ...
        window.open("http://applications.fairview.org/PhoneDirectory/StaffSearch.asp?Last="+SearchStrings);
                ...
                $('#PagerSearchButton2').click(function () {
            var SearchStrings = $('#PagerLastName').val()+'&First='+$('#PagerFirstName').val();
