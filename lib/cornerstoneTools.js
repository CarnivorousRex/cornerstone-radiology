...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onTouch(e) {
		...
		// If clicking on the text box for the thumbnails, 
		var thumbnail = $(".csthumbnail").get(0);
		var listItem = $(".list-group-item").get(0);

		if (listItem.isEqualNode(e.target.parentNode)) {
			element = thumbnail;
			console.log("bad thing tapped");
			return false;
		}
		
        // Prevent mouse events from occurring alongside touch events
		// unless trying to scroll the thumbnails


		
		if (!thumbnail.isEqualNode(e.target.parentNode)) {
			//if (!listItem.isEqualNode(e.target.parentNode)) {
			//console.log("not a thumbnail");
			e.preventDefault();
			//}
		}
        ...
        switch (e.type) {
            ...
            case 'doubletap':
				...
				// Doubletap the viewport for fullscreen, doubletap again to exit
				cornerstoneTools.makeFullscreen();
		...
		//  Don't return false if trying to scroll a thumbnail
		var thumbnail = $(".csthumbnail").get(0);
		if (!thumbnail.isEqualNode(e.target.parentNode)) {
			return false;
		} else { 
			return; 
		}  // end false prevention
    }
    ...
    function enable(element) {
		...
		var thumbnail = $(".thumbnailSelector").get(0);
		
		if (!thumbnail.isEqualNode(element.parentNode)) {
        	mc.on('press tap doubletap panstart panmove panend pinch rotate', onTouch);
		} else {
			mc.on('press tap doubletap pinch rotate', onTouch);
		}
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
		...
		var foo = $("div.viewport.ui-droppable").get(0);
		console.log(eventData.element == foo);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
...
(function(cornerstone, cornerstoneTools) {
    ...
    function keyDownCallback(e, eventData) {
		...
		if (keyCode >= 49 && keyCode <= 56) {
			...
			//var element = cornerstoneTools.getEnabledElement;
			//var canvasElement = $(".viewport > canvas").get(0);
			//var foo2 = cornerstone.getEnabledElement(canvasElement);
			//var foo = canvasElement.getContext("2d");
...
console.log(viewport);
			...
			if (keyCode == 50) {
			    viewport.voi.windowWidth = 30;
                viewport.voi.windowCenter = 30;
				cornerstone.setViewport(element, viewport);				
			}			
			if (keyCode == 51) {
			    viewport.voi.windowWidth = 125;
                viewport.voi.windowCenter = 75;
				cornerstone.setViewport(element, viewport);				
			}			
			if (keyCode == 52) {
...
(function($, cornerstone, cornerstoneTools) {
	function makeFullscreen() {
		...
		if ($.fullscreen.isFullScreen() === true) {	
			//already fullscreen
			$("#fullscreen").removeClass("btn-fullscreen");
			// Stuff to undo the resizing on a small screen
			if ($(window).width() < 767) {
				$("body").removeClass("fullscreen-style");
				$(".viewer").removeClass("fullscreen-style");
				$("div.viewer").removeClass("fullscreen-style");
				$("div.viewport canvas").removeClass("fullscreen-style");
				$("div.viewport canvas").removeClass("center");
				$(".thumbnails").removeClass("hidden");
				$("ul.nav-tabs").eq(0).removeClass("hidden");
				//cornerstone.fitToWindow(element);
			} 
			$.fullscreen.exit();
		} else {
		// not yet fullscreen, now set fullscreen
			$("#fullscreen").addClass("btn-fullscreen");
			$("#scrollButton").click();
			if ($(window).width() < 767) {
				//console.log(element);
				// Stuff just for the resizing on a small screen
				//$("body").eq(0).addClass("fullscreen-style");
				$(".viewer").eq(0).addClass("fullscreen-style");
				$("div.viewport canvas").eq(0).addClass("fullscreen-style");
				$("div.viewport canvas").eq(0).addClass("center");
				$(".thumbnails").addClass("hidden");
				$("ul.nav-tabs").eq(0).addClass("hidden");
				$(".viewer").fullscreen();
				var canvasElement = $(".viewport > canvas");
				//cornerstone.getEnabledElement(canvasElement); 
				
				//cornerstone.fitToWindow(element);
			} else {
				// Resize a larger screen
				console.log("Resizing a larger screen");
    ...
    // Module exports
    cornerstoneTools.makeFullscreen = makeFullscreen;
